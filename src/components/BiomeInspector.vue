<template>
  <div class="biomeInspector">
    <div class="biomes">
      <div class="biomesTitle">生物群系</div>
      <BiomeList :selectedBiome="biome" />
    </div>
    <div class="info">
      <h2 class="title">{{biome.name}} 生物群系</h2>
      <h3 class="subtitle" v-if="!biome.data">加载中...</h3>
      ...
      <ul v-if="biome.data">
        <li>
          温度: {{temperatureText}}
          <span class="details">(地面热量: {{biome.data.groundHeat}})</span>
        </li>
      </ul>
      <div class="objects">
...
export default {
  ...
  computed: {
    ...
    temperatureText() {
      ...
      if (heat < 0.0) return "严寒";
      if (heat < 0.5) return "寒冷";
      if (heat < 1.0) return "凉爽";
      if (heat < 1.1) return "适中";
      if (heat < 1.9) return "温暖";
      if (heat < 2.1) return "炎热";
      return "酷热"
    }
  },
  metaInfo() {
